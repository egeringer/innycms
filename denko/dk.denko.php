...
class Denko{
    ...
    public static function openDB($iniPath=null){
        ...
        if($iniPath===null){
            $iniPath=dirname(__FILE__).'/../DB.ini.local';
            if(!file_exists($iniPath)){
                $iniPath = dirname(__FILE__).'/../DB.ini';
            }
        }
        if(strpos($iniPath,"ini")){
            $config = parse_ini_file($iniPath,TRUE);
        } else if(strpos($iniPath,"json")){
            $file = file_get_contents($iniPath);
...
/**
    ...
    public static function str_to_friendlyUrl($string){
        ...
        $string = Denko::lower($string);
        $string = preg_replace('/[^A-Za-z0-9-]+/', '-', $string);
        $string = preg_replace('/[\-]{2,}/','-',$string);
        return trim(str_replace("--","-",$string),"-");
    }

    public static function bytesToFriendlyUnit($bytes){
        if ($bytes >= 1073741824)
        {
    ...
    */
    ...
    /*
     * Funciones que usara el plugin dk_include.
     * De esta forma se podran hacer includes de hojas de estilo y javascript desde php.
     * Smarty simplemente usara las variables globales.
     */
    private static function smarty_include_verify_included_file($filepath){
    ...
    private static function smarty_include_create_compressed_file($file_path){

        #
        $originalFile_basename = basename($file_path);
        $originalFile_time = filemtime($file_path);

        # Obtengo el path del archivo en su versiÃ³n comprimida
        $new_filepath = 'templates_c/compressed.'.$originalFile_basename;

        # En caso que el archivo no exista, lo genero
        if(!file_exists($new_filepath) || $originalFile_time > filemtime($new_filepath)){
            # Genero el archivo comprimido
            require_once dirname(__FILE__).'/../vendor/minify/JSMin.php';
            $js_minified = trim(JSMin::minify(file_get_contents($file_path)));
            file_put_contents($new_filepath,$js_minified);
        }
    }

    public static function smarty_include($file,$inline = "auto",$compress = true,$ignoreVersion = '',$version = '',$media = '',$extParams = array()){

        # Obtengo la informaciÃ³n del path
        ...
        $filepath = $file;
        if(empty($ignoreVersion)){
            # Verifico si el archivo debe ser comprimido
            $compress = (($compress === true) || (strtolower($compress) === 'true'));

            # La versiÃ³n puede estar seteada entre los parÃ¡metros.
            # En caso contrario, genero la versiÃ³n tomando en cuenta la fecha de actualizaciÃ³n del archivo
            if(!file_exists($file)) self::fatalError("No existe el archivo $file.");

            $version = !empty($version) ? $version : (date('ymdHis',filemtime($file)));
            $filepath = $file_directory.'-'.$version;

            # La compresiÃ³n de archivos por ahora estÃ¡ habilitada para los javascript
            if($compress == true && $file_extension == 'js'){
                Denko::smarty_include_create_compressed_file($file);
                $filepath .= '-c';
            }
        ...
        # Obtengo los parÃ¡metros extra que se setearon en el plugin
        $extraParams = '';
        foreach($extParams as $param => $value){
        ...
        switch($file_extension){
            ...
            case 'css':
                # En caso de no existir el parÃ¡metro 'media', asumo media="screen"
        ...
        $inline = !empty($inline) ? (is_string($inline) ? strtolower($inline) : $inline) : 'auto';

        # En caso que el plugin deba comportarse como siempre
        if($inline == 'auto'){
            echo Denko::smarty_include_verify_included_file($file) ? $html : '';
            return '';
        }

        # En caso que el cÃ³digo HTML deba ir en el header
        if($inline == 'false' || $inline === false){

            # Agrego el HTML en el arreglo de includes
            if(Denko::smarty_include_verify_included_file($file)){
                if(!isset($GLOBALS['DENKO_INCLUDES'])){
                    $GLOBALS['DENKO_INCLUDES'] = array();
                }
                $GLOBALS['DENKO_INCLUDES'][] = $html;
            }
        ...
        # En caso que el cÃ³digo HTML deba ser retornado
