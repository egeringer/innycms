...
$('input').on('itemAdded', function(event) {
    ...
    var id = ids.split("-")[1];
    $.post( "bucket", { action: 'addtag', id_bucket: id, tag: event.item } );
}).on('itemRemoved', function(event) {
    var ids = event.currentTarget.id;
    var id = ids.split("-")[1];
    $.post( "bucket", { action: 'removetag', id_bucket: id, tag: event.item } );
});
...
$('#deleteItem').on('show.bs.modal', function(e) {
    ...
    button.on("click",function(e){
        e.preventDefault();
        var url = "./bucket-delete:"+id+"x"+hash+"?from="+from;
        $.ajax({
            url: url,
            type: 'DELETE',
